...
class Rekening extends \Aksara\Laboratory\Core
{
	...
	public function index()
	{
		...
		$this->set_title('Rekening Pembiayaan, non anggaran/PFK, rekening koran (RK) dan BLUD')
	...
	private function _filter()
	{
		$output										= null;
		$query										= $this->model
                                                    ->select('MATANGB.KDPER, MATANGB.NMPER, MATANGB.MTGLEVEL')
                                                    ->order_by('MATANGB.KDPER ASC')
                                                    ->get_where('MATANGB', array('MATANGB.MTGLEVEL' => 1))
                                                    ->result();
		if($query)
		{
